...
module.exports = {
    run: async ({ interaction }) => {
        ...
        try {
            ...
            if(interaction.channelId != 1229572193902530580){
            ...
            if(userFishBattleStats.startedFB == 1){
                interaction.reply({
                    content: "You've already started a fish battle. Please wait for that one to end.",
                    ephemeral: true,
                });
                return;
            }

            if(targetFishBattleStats.startedFB == 1){
                interaction.reply({
                    content: "You've already started a fish battle. Please wait for that one to end.",
                    ephemeral: true,
                });
            ...
            if(targetFishBattleStats.inFB == 1){
                interaction.reply({
                    content: "This user is already in a fish battle. Please wait for them to finish.",
                    ephemeral: true,
                });
                return;
            }
            ...
            const fishBattleEmbed = new EmbedBuilder()
                .setTitle(`🎣 **${rolls}x FISH BATTLE: $${(amount_bet).toLocaleString("en-US")}** ⚔️`)
            ...
            userFishBattleStats.startedFB = 1;
            await Promise.all([userFishBattleStats.save()]);
            ...
            const targetUserInteraction = await reply.awaitMessageComponent({
                ...
                userFishBattleStats.startedFB = 0;
                await Promise.all([userFishBattleStats.save()]);
            ...
            if(targetUserChoice.name == "Fish"){
                userFishBattleStats.startedFB = 1;
                targetFishBattleStats.startedFB = 1;
                targetFishBattleStats.inFB = 1;
                await Promise.all([targetUserProfile.save(), userProfile.save(), userFishBattleStats.save(), targetFishBattleStats.save()]);
                ...
                if(targetUserProfileFixed.balance < amount_bet){
                    userFishBattleStats.startedFB = 0;
                    targetFishBattleStats.startedFB = 0;
                    targetFishBattleStats.inFB = 0;
                    await Promise.all([targetUserProfile.save(), userProfile.save(), userFishBattleStats.save(), targetFishBattleStats.save()]);
                    await reply.edit({
            ...
            const initialUserInteraction = await reply.awaitMessageComponent({
                ...
                targetFishBattleStats.inFB = 0;
                targetFishBattleStats.startedFB = 0;
                await Promise.all([userFishBattleStats.save(), targetFishBattleStats.save()]);
                targetUserProfile.balance += amount_bet;
                targetUserProfile.balance += amount_bet;
                await Promise.all([targetUserProfile.save()]);
                const endEmbed = new EmbedBuilder()
